...
exports.createSeason = async (req, res) => {
  try {
    const { name, startDate, farmId } = req.body;
    if (!name || !startDate) {
    ...
    const season = new Season({ name, startDate, userId: req.user.userId, farmId: farmId || null, endDate: null });
    await season.save();
...
exports.getAllSeasons = async (req, res) => {
  try {
    ...
    const seasons = await Season.find(filter).sort({ startDate: -1 });
    if (!seasons.length) {
